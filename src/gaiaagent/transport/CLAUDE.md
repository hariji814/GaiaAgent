...
- **Inbound body limits are not yet enforced** (TODO P1 ingress hardening): no max body size, no connection-count cap, no ingress rate limit, and error paths may leak `str(exc)`. When you close one of these, route errors through a structured mapping layer — never echo raw exception text to the wire.
